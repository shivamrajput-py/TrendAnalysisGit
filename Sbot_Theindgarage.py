'''DATA SCRAPPING BOT [THE INDIAN GARAGE] FOR RAWCULT TREND-ANALYSIS
~ SHIVAM RAJPUT '''

# SORTING DICTIONARY ACCORDING TO THE SOURCE LINK
sort_dict = {
...
MAX_PRODUCT_FROM_EACH_CATEGORY = NO_OF_PRODUCTS_TO_SCRAPE['TheIndianGarage']
URL_DICT = TO_SCRAPE_URL_DICT['TheIndianGarage']
...
for SUB_CATEGORY in URL_DICT:
    ...
    for sort_type in sort_dict.keys():
        ...
        driver.get(URL_DICT[SUB_CATEGORY].replace('best-selling', sort_dict[sort_type]))
        ...
        print(f'[THE INDIAN GARAGE] >> {SUB_CATEGORY} >> {sort_type}\n')
        ...
        for j, elm in enumerate(driver.find_elements(By.CSS_SELECTOR, '.collection_product_item.product-item')):

            if (j+1) > MAX_PRODUCT_FROM_EACH_CATEGORY: break

            try:
                productList.append(elm.find_element(By.XPATH, 'div[1]/a').get_attribute('href'))
            except:
                try:
                    productList.append(elm.find_element(By.XPATH, 'div[1]/a').get_attribute('href'))
                except:
        ...
        for product in productList:
            ...
            # IMAGE LINK *
            try:
                img_link = driver.find_element(By.CSS_SELECTOR, ".product__item_grid").find_element(By.XPATH, "div/a").get_attribute('href')
            except:
            ...
            # TITLE *
            try:
                title = driver.find_element(By.CSS_SELECTOR, '.product-meta__title.heading.h3.product-detail__title').text
            except:
            ...
            # CURRENT PRICE *
            try:
                price = driver.find_element(By.CSS_SELECTOR, '.price.price--highlight.price--large').text
            ...
            # RATING OUT OF 5
            try:
                rating = driver.find_element(By.CSS_SELECTOR, '.pdp-product-ratingsContainer').text.split('|')[0].strip()
            ...
            try:
                rating_info = driver.find_element(By.CSS_SELECTOR, '.pdp-product-ratingsContainer').text.split('|')[1].strip().split('Reviews')[0]
                try:
            ...
            revwDict = {}
            try:
                for i,rev_div in enumerate(driver.find_elements(By.CSS_SELECTOR, ".user-review-reviewTextWrapper")):
                    if i==3: break
            ...
            attributes = []
            try:
                for j in driver.find_elements(By.CSS_SELECTOR, '.custom-styled-div'):
...
smpdt = FINALDATA[SUB_CATEGORY]['Recommended']
for dt in smpdt: dt['sorting'] = 'Popularity'
FINALDATA[SUB_CATEGORY]['Popularity'] = smpdt

# MAKING THE JSON FILE FO THE FINAL DATA
with open(f'prodData_TheIndgarage.json', 'w', encoding="utf-8") as fl:
...
try:
    ...
    print('SCRAPPED THE INDIAN GARAGE!\n')
