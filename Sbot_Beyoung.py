...
sort_dict = {
    'Recommended': '',
    'Freshness': '?order=newly-ASC',
    'Popularity': '?order=trending-ASC'
}
...
for SUB_CATEGORY in URL_DICT:
    ...
    for sort_type in sort_dict.keys():
        ...
        print(f'[BEYOUNG] >> {SUB_CATEGORY} >> {sort_type}\n')
        ...
        for q, elm in enumerate(driver.find_elements(By.CSS_SELECTOR, '.products')):
        ...
        for product in productList:
            ...
            # IMAGE LINK *
            try:
                img_link = driver.find_element(By.CSS_SELECTOR, '.slider-banner-image').find_element(By.TAG_NAME, 'img').get_attribute('src')
            except:
            ...
            # TITLE *
            try:
                title = driver.find_element(By.CSS_SELECTOR, '.p-heart-icon').text
            except:
            ...
            # CURRENT PRICE *
            try:
                price = driver.find_element(By.CSS_SELECTOR, '.realprice').text
            ...
            # ORIGINAL PRICE
            try:
                oprice = driver.find_element(By.CSS_SELECTOR, '.cuttinprice').text

                # EXTRACTING
                oprice = oprice.replace('₹', '').replace(',', '').strip()
            ...
            # RATING INFO
            try:
                rating_info = driver.find_element(By.CSS_SELECTOR, '.pdp-top-rating').text.split('\n')[1]
                ratings_count = int(convert_to_number(rating_info.lower()))
            except:
                ratings_count = None

            # RATINGS NUMBER
            try:
                rating = driver.find_element(By.CSS_SELECTOR, '.pdp-top-rating').text.split('\n')[0]
            ...
            # REVIEWS COUNT
            try:
                reviews_count = driver.find_element(By.CSS_SELECTOR, '.viewallreview-text').text
                reviews_count = reviews_count.lower().replace('reviews', '').replace('all', '').replace('view', '').strip()
                try: reviews_count = int(reviews_count)
                except: reviews_count = reviews_count
            except:
                reviews_count = None
            ...
            attributes = []
            try:
                attributes = driver.find_element(By.CSS_SELECTOR, '#__next > div.product-page-main-div > section.container.shirt-product-main > div > div.product-details-info > div.pdp-description > div > div.pdpspecifications.active').text.split('\n')
            except:
                attributes = []
