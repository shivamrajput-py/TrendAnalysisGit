...
for SUB_CATEGORY in URL_DICT:
    ...
    for sort_type in sort_dict.keys():
        ...
        driver.get(URL_DICT[SUB_CATEGORY] + f'?sort_by={sort_dict[sort_type]}')
        ...
        print(f'[PRONK] >> {SUB_CATEGORY} >> {sort_type}\n')
        ...
        for q,elm in enumerate(driver.find_elements(By.CLASS_NAME, 'grid-product__link')):
        ...
        for product in productList:
            ...
            # IMAGE LINK *
            # "#ProductPhotos-template--16362792615980__main > div > div > div.product-main-slide.starting-slide.is-selected > div > div > image-element > img"
            try:
                img_link = driver.find_element(By.CSS_SELECTOR, '.image-wrap').find_element(By.XPATH, 'image-element/img').get_attribute('src')
            except:
            ...
            # ORIGINAL PRICE
            try:
                oprice = driver.find_element(By.CSS_SELECTOR, '.product__price.product__price--compare').text
            ...
            # RATING INFO
            try:
                rating_info = driver.find_element(By.CSS_SELECTOR, '.jdgm-prev-badge__text').text

                # EXTRACTING REQUIRED DATA.
                rating_counts = rating_info.split('|')[1].strip()
            ...
            try:
                rating_info = driver.find_element(By.CSS_SELECTOR, '.jdgm-prev-badge__text').text
                rating = rating_info.split('|')[0].strip()
            ...
            attributes = []
            try:
                text_box = driver.find_element(By.CSS_SELECTOR, '.rte').text.split('\n')
            except:
                attributes = []
...
with open(f'prodData_Pronk.json', 'w', encoding="utf-8") as fl:
