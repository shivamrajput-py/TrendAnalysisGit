...
for SUB_CATEGORY in URL_DICT:
    ...
    for sort_type in sort_dict.keys():
        ...
        driver.get(URL_DICT[SUB_CATEGORY].replace('manual', sort_dict[sort_type]))
        ...
        print(f'[CAMPUS SUTRA] >> {SUB_CATEGORY} >> {sort_type}\n')
        ...
        for j, elm in enumerate(driver.find_elements(By.CLASS_NAME, 'product-item ')):
        ...
        for product in productList:
            ...
            # IMAGE LINK *
            try:
                img_link = driver.find_element(By.CSS_SELECTOR, ".flickity-slider").find_element(By.XPATH, "div/div/img").get_attribute('src')
            except:
            ...
            # TITLE *
            try:
                title = driver.find_element(By.CSS_SELECTOR, '.product-meta__title.heading.h3').text
            except:
            ...
            try:
                ...
                if 'Rs.' in str(price): price = price.split('Rs.')[1].strip().split(' ')[0]
            ...
            # RATING OUT OF 5
            try:
                rating = driver.find_element(By.CSS_SELECTOR, '.jdgm-rev-widg__summary-average').text
            ...
            try:
                rating_info = driver.find_element(By.CSS_SELECTOR, '.jdgm-rev-widg__summary-text').text
                rating_info = rating_info.split('on')[1].strip().split('reviews')[0].strip()
                try:
            ...
            attributes = []
            try:
                for j in driver.find_elements(By.CSS_SELECTOR, '.main_inn'):
...
with open(f'prodData_Campusutra.json', 'w', encoding="utf-8") as fl:
