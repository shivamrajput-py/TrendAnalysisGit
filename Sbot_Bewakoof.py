''' DATA SCRAPPING BOT [BEWAKOOF] FOR RAWCULT TREND-ANALYSIS
~ SHIVAM RAJPUT '''
...
MAX_PRODUCT_FROM_EACH_CATEGORY = NO_OF_PRODUCTS_TO_SCRAPE['Bewakoof']
URL_DICT = TO_SCRAPE_URL_DICT['Bewakoof']
HEADLESS_BROWSER = False
scroll_pause_time = 1.85 # According to your Internet Speed
IMPLICIT_WAIT = 0.5
...
for SUB_CATEGORY in URL_DICT:
    ...
    for sort_type in sort_dict.keys():
        ...
        print(f'[BEWAKOOF] >> {SUB_CATEGORY} >> {sort_type}\n')
        ...
        for i,prodbox in enumerate(driver.find_elements(By.XPATH, '//*[@id="__next"]/main/main/div[2]/div/main/div/section[2]/section/section')):
        ...
        for product in productList:
            ...
            # IMAGE LINK *
            try:
                img_link = driver.find_element(By.CSS_SELECTOR, '.swiper-zoom-container').find_element(By.TAG_NAME, 'img').get_attribute('src')
            except:
            ...
            # TITLE *
            try:
                title = driver.find_element(By.XPATH, '/html/body/div[1]/main/main/div[1]/div[1]/section[2]/div[1]/div[1]/span').text
            except:
            ...
            # CURRENT PRICE *
            try:
                price = driver.find_element(By.CSS_SELECTOR, '.flex.items-baseline').text.split('\n')[0]
            ...
            # ORIGINAL PRICE
            try:
                oprice = driver.find_element(By.CSS_SELECTOR, '.flex.items-baseline').text.split('\n')[1]
            ...
            # WRITTEN REVIEWS COUNTINGS
            try:
                reviews_count = driver.find_element(By.CSS_SELECTOR,
                                                    'div[data-testid="pdp-product-rating"]').text.split('\n')
                if len(reviews_count) == 3:
                    reviews_count = int(convert_to_number(reviews_count[2].replace('Reviews', '')))
                else:
                    reviews_count = None
            ...
            time.sleep(IMPLICIT_WAIT+0.8)

            try:
                driver.find_element(By.CSS_SELECTOR, 'button[data-testid="pdp-product-review-tab"]').click()
                time.sleep(0.35)
            except:
                pass
            ...
            # RATINGS NUMBER
            try:
                rating = driver.find_element(By.CSS_SELECTOR, '.sc-660da3ca-1.lfYUXc').text.split('\n')[0]
                ...
                try:
                    rating = driver.find_element(By.CSS_SELECTOR,
                                                 '#__next > main > main > div.sc-e539384c-1.iFotXi > div.sc-4efc2775-0.hZEEzx > section.sc-4efc2775-3.eOSYUQ > div:nth-child(9) > div:nth-child(2) > div > div.px-\[15px\] > div.sc-660da3ca-0.dnLVEi > div.sc-660da3ca-1.lfYUXc > span.text-\[2rem\]').text
                    # EXTRACTING
                    if rating.replace('.', '').isnumeric():
                        rating = round(float(rating), 1)
                    else:
            ...
            # RATING INFO
            try:
                rating_info  = driver.find_element(By.CSS_SELECTOR, '.sc-660da3ca-1.lfYUXc').text.split('\n')[1]

                # EXTRACTING REQUIRED DATA.
                ratings_count = rating_info.lower().replace('ratings', '').replace('+', '').strip()
                ratings_count = int(convert_to_number(ratings_count.upper()))
            except:
                try:
                    rating_info = driver.find_element(By.CSS_SELECTOR, r'#__next > main > main > div.sc-e539384c-1.iFotXi > div.sc-4efc2775-0.hZEEzx > section.sc-4efc2775-3.eOSYUQ > div:nth-child(9) > div:nth-child(2) > div > div.px-\[15px\] > div.sc-660da3ca-0.dnLVEi > div.sc-660da3ca-1.lfYUXc > span.sc-f48c17b3-0.eQRJvu').text
                    # EXTRACTING REQUIRED DATA.
                    ratings_count = rating_info.split('ratings')[0].strip().replace('+', '').strip()
                    ratings_count = int(convert_to_number(ratings_count.lower()))
                except:
                    ratings_count = None
            ...
            attributes = []
            try:
                for j in driver.find_elements(By.CSS_SELECTOR, '.sc-1f8735d0-3.jRWMiC'):
            ...
            revwDict = {}
            # try:
            #     for i, rev in enumerate(driver.find_elements(By.CSS_SELECTOR, '.sc-52aa9520-0.fKrKsa')):
            #         revwDict[i+1] = rev.text.strip()
            # except:
            #     revwDict = {}
            print(ratings_count, rating)
...
with open(f'prodData_Bewakoof.json', 'w', encoding="utf-8") as fl:
